...
export function DesignerSidebar({ designerState, updateDesignerState }: DesignerSidebarProps) {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              const address = data.formatted_address || `${lat.toFixed(6)}, ${lng.toFixed(6)}`
              const irradiance = calculateIrradiance(lat, lng)
        ...
        (error) => {
          ...
          alert('Konum bilgisi alınamadı. Lütfen tarayıcınızda konum izni verildiğinden emin olun.')
        },
        {
  ...
  // Helper function to calculate irradiance based on coordinates
  const calculateIrradiance = (lat: number, lng: number): number => {
    // Turkey-specific irradiance calculation
    let baseIrradiance = 1450
    
    if (lat >= 36 && lat < 37) {
      baseIrradiance = 1750 + (37 - lat) * 100
    } else if (lat >= 37 && lat < 38) {
      baseIrradiance = 1650 + (38 - lat) * 80
    } else if (lat >= 38 && lat < 39) {
      baseIrradiance = 1550 + (39 - lat) * 60
    } else if (lat >= 39 && lat < 40) {
      baseIrradiance = 1450 + (40 - lat) * 40
    } else if (lat >= 40 && lat < 41) {
      baseIrradiance = 1350 + (41 - lat) * 50
    } else if (lat >= 41 && lat < 42) {
      baseIrradiance = 1250 + (42 - lat) * 30
    }
    
    // Longitude adjustment for east-west variations
    if (lng >= 26 && lng <= 45) {
      const lngFactor = 1 + (lng - 35) * 0.01
      baseIrradiance *= lngFactor
    }
    
    return Math.round(baseIrradiance)
  }

  // Address search with Places API via server endpoint
  const handleAddressSearch = async (query: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAddressSuggestions(suggestions.map((s: any) => s.description))
      }
    } catch (error) {
              ...
              <div className="relative">
                ...
                <div className="flex space-x-2 mt-1">
                  <div className="relative flex-1">
                    <Input 
                      id="address" 
                      placeholder="İl, İlçe, Mahalle... (örn: Kırklareli, İstanbul)"
                      className="text-sm"
                      value={addressInput}
                      onChange={(e) => {
                  ...
                  <Button size="sm" onClick={handleLocationSearch} title="Mevcut konumu kullan">
