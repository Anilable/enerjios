...
export function GoogleMapsDesigner({
  ...
  const onPolygonComplete = (polygon: google.maps.Polygon) => {
    ...
    setRoofArea(validatedArea)
    
    // Check license limits for roof area
    const roofAreaCheck = checkRoofAreaLimit(validatedArea)
    if (!roofAreaCheck.allowed) {
      setLicenseError(roofAreaCheck.message || 'Çatı alanı limiti aşıldı')
      polygon.setMap(null)
      setRoofPolygon([])
      setRoofArea(0)
      return
    ...
    // Calculate panel placement automatically
    const panelWidth = 2.0 // meters
    const panelHeight = 1.0 // meters
    const panelArea = panelWidth * panelHeight
    const panelPower = 450 // watts per panel
    
    // Calculate usable area (70% of roof area for obstacles, edges, etc.)
    const usableArea = Math.round(validatedArea * 0.7)
    const maxPanels = Math.floor(usableArea / panelArea)
    
    // Check license limits for panel count
    const panelCheck = checkPanelLimit(maxPanels)
    const allowedPanels = panelCheck.allowed ? maxPanels : (licenseInfo?.maxPanels || maxPanels)
    ...
    >
      <div className="relative w-full h-full">
        ...
        >
          ...
          {roofPolygon.length > 0 && showPanelGrid && showLayers.panels && designerState.calculations.totalPanels > 0 && (
            <>
              {/* Create a simplified panel grid visualization */}
              {Array.from({ length: Math.min(designerState.calculations.totalPanels, 20) }, (_, index) => {
                // Calculate panel positions within the roof polygon bounds
                const bounds = roofPolygon.reduce((acc, point) => ({
                  minLat: Math.min(acc.minLat, point.lat),
                  maxLat: Math.max(acc.maxLat, point.lat),
                  minLng: Math.min(acc.minLng, point.lng),
                  maxLng: Math.max(acc.maxLng, point.lng)
                }), {
                  minLat: roofPolygon[0].lat,
                  maxLat: roofPolygon[0].lat,
                  minLng: roofPolygon[0].lng,
                  maxLng: roofPolygon[0].lng
                })
                
                const cols = Math.ceil(Math.sqrt(designerState.calculations.totalPanels))
                ...
                return (
                  <Polygon
                    key={`panel-${index}`}
                    paths={[
                      { lat: panelLat - latStep * 0.3, lng: panelLng - lngStep * 0.15 },
                      { lat: panelLat - latStep * 0.3, lng: panelLng + lngStep * 0.15 },
                      { lat: panelLat + latStep * 0.3, lng: panelLng + lngStep * 0.15 },
                      { lat: panelLat + latStep * 0.3, lng: panelLng - lngStep * 0.15 }
                    ]}
                  ...
                  {roofPolygon.length > 0 && (
                    <>
                      ...
                      >
                        ...
                        {showPanelGrid ? 'Panelleri Gizle' : 'Panelleri Göster'}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
